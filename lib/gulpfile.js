...
gulp.task('ts-lint', (done) => {
  ...
  const args = [tslintBin, '-c', tslintConfig, 'components/**/*.tsx', '-t', 'checkstyle', '-o', 'checkstyle-tslint.xml'];
...
function compile(modules) {
  ...
  const less = gulp.src(['components/**/*.less'])
    .pipe(through2.obj(function (file, encoding, next) {
      this.push(file.clone());
      if (file.path.match(/\/style\/index\.less$/) || file.path.match(/\\style\\index\.less$/) || file.path.match(/\/style\/v2-compatible-reset\.less$/) || file.path.match(/\\style\\v2-compatible-reset\.less$/)) {
